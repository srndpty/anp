...
**Phase 2 を進行中。ページ色変換（オリジナル / 反転）、キャンバス配色、
UI テーマまで完了。**
...
| Phase 2 | ページ色変換（反転 → Smart Dark）、キャンバス配色、UI テーマ | 進行中 |
...
- 全画面表示
- **ページの色を反転**（表示メニュー → ページの色）。反転するのは PDF の
  ページ画像だけで、キャンバス・ツールバー・メニューは元のまま
...
パスワード付き PDF を開くこと、Smart Dark（色味を補正したダークモード）、
